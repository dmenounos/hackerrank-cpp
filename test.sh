...
main() {
	if [ -z "$1" ] || [ -z "$2" ]; then
		echo "Usage: $0 <executable_file> <test_data_path>" >&2
		exit 1
	fi
	...
	if [ ! -f "$EXEC_FILE" ]; then
		echo "Could not resolve test executable file." >&2
		exit 1
	fi

	if [ ! -d "$TEST_DATA_FOLD" ]; then
		echo "Could not resolve test data directory." >&2
		exit 1
	fi
...
test() {
	...
	if [ ! -f "$TEST_DATA_FOLD/$TEST_INPUT_FILE" ]; then
		echo "Could not resolve expected test data file: $TEST_DATA_FOLD/$TEST_INPUT_FILE" >&2
		exit 1
	fi

	if [ ! -f "$TEST_DATA_FOLD/$TEST_OUTPUT_FILE" ]; then
		echo "Could not resolve expected test data file: $TEST_DATA_FOLD/$TEST_OUTPUT_FILE" >&2
		exit 1
	fi
